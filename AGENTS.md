...
### Tables
- `dict_metric` — 指标字典（id, category_id, metric_name, unit）
- `dict_category` — 指标分类（id, dept_name, category_name）
- `dept_stats` — 部门统计数据（metric_id, dept_name, year, value）

### Key Departments (dict_category.dept_name)
